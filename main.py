...
def collision_avoidance(count):
    ...
    for asteroid in asteroids:
        ...
        if collision_distance(player.x, player.y, asteroid.x, asteroid.y) < 150:
            player_angle = player.angle % 360;
            print("avoiding collision")
            if asteroid.xv >= 0 and asteroid.yv >= 0:
                # print(f'Asteroid vel: {asteroid.xv}, {asteroid.yv} turning right')
                if(0 <= player_angle <= 90 or 180 <= player_angle <= 270):
                    player.turnRight()
            elif asteroid.xv >= 0 and asteroid.yv < 0:
                # print(f'Asteroid vel: {asteroid.xv}, {asteroid.yv} turning left')
                if(90 <= player_angle <= 180 or 270 <= player_angle <= 359):
                    player.turnLeft()
            elif asteroid.xv < 0 and asteroid.yv >= 0:
                # print(f'Asteroid vel: {asteroid.xv}, {asteroid.yv} turning left')
                if(90 <= player_angle <= 180 or 270 <= player_angle <= 359):
                ...
                if(0 <= player_angle <= 90 or 180 <= player_angle <= 270):
                    player.turnRight()
            if(count%2==0): player.moveForward()
            collision = True

    return collision

def collect_powerupV2(star, count):

    angle = (270 - (math.atan2( (star.y + star.h // 2) - player.y, (star.x + star.w // 2) - player.x ) * ( 180 / 3.141592 ) % 360)) % 360
    
    # print(f'count : {count} player angle : {player.angle % 360} angle to star {angle}')
    # print(f'player pos : {player.x}, {player.y} star pos : {star.x}, {star.y}')

    val = player.angle - angle
    if(abs(val) > 10):
        if(count % 10 == 0): 
            player.turnLeft()
    elif(count % 2 == 0):
...
def collect_powerup(time_elapsed):
    ...
    # print(f'star angle {star_kinematic.orientation} | player angle {player_kinematic.orientation}')
    ...
    # if(-10 < star_kinematic.orientation - player_kinematic.orientation < 10):
...
while run:
    ...
    if not gameover:
        ...
        collecting_powerup = False
        if count % 50 == 0:
            ran = random.choice([1, 1, 1, 2, 2, 3])
            asteroids.append(Asteroid(ran))
        if count % 1000 == 0 and count > 0 and stars == [] and not rapidFire:
            stars.append(Star())
            # collect_star = True
        ...
        collision_flag = collision_avoidance(count)
        ...
        for a in asteroids:
            ...
            #         # print("Collision")
            #         # print(f'Asteroid vel: {a.xv}, {a.yv}')
            #         lives -= 1
        ...
        for s in stars:
            ...
            player_kinematic.position = pymath.Vector2(player.x, player.y)
            player_kinematic.orientation = player.angle % 360

            theta = math.atan2(s.y - player.y, s.x - player.x)
            deg = theta * 180 / 3.141592
            star_kinematic.orientation = deg % 360
            ...
            collecting_powerup = True

            if argv[1] == 'ARBITRATION' and not collision_flag:
                collect_powerupV2(s, count)
            elif argv[1] == 'BLENDING':
                collect_powerupV2(s, count)
            ...
            if (collision_distance(player.x, player.y, s.x + s.w//2, s.y + s.h//2) < 20 ):
                rapidFire = True
                rfStart = count
                stars.pop(stars.index(s))
                # playerBullets.pop(playerBullets.index(b))
        ...
        if not gameover:
            ...
            if count % 5 == 0 and not collecting_powerup and not collision_flag:
                if rand_dir:
                    player.turnLeft()
                else:
                    player.turnRight()
